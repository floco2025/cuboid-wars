...
| Scuttlers, bruisers, zappers, and turrets | Luring, containment, guarded space, exposure windows, and resource use. The actor named bruiser is a contact attacker; zappers provide mobile ranged guards and turrets provide stationary continuous-fire guards. |
...
8. **Containment:** Open the green start barriers to release both the player and bruiser. Open the cyan pen from the corridor switch and lead the bruiser inside. Collect the amber exit key, climb the pen's escape ladder, drop outside, and close the pen using the second cyan switch. Return through the bruiser's area and use the key at the exit gate. Contact with the hunter is lethal at full health; the closed field shields against its blast. The player can currently outrun the bruiser and finish without trapping it.
