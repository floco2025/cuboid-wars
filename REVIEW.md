...
R1 and R5–R8 have been implemented with focused behavioral regressions and removed from TODO. R2–R4 are accepted behavior and have also been removed from Fixes; network testing now targets convergence and existing explicit guarantees. Additional test/CI coverage was declined. Dependency centralization is complete and removed from TODO. Any shared-map extraction requires separate evaluation.
...
Startup presentation follow-up: an opaque loading screen now hides the progressive scene build until textures, model instances, nearby grass, and shader pipelines are ready. Hotel and Obby were captured from loading through the completed scene. A temporary missing-texture check kept the error screen visible, and Esc exited successfully both during normal loading and after the failure; the asset catalog was restored. All 524 existing client tests and workspace/all-target Clippy passed; no tests were added.

Startup flash correction: the cover now draws above the scene image and HUD in their shared UI pass. The previous separate camera depended on its own output pipeline becoming ready, allowing a quest banner to appear first. Two Linux Hotel startup runs showed the black/loading cover followed by the completed scene and quest banners. The earliest captured game window was black at about 0.54 seconds; screenshots sample startup rather than establishing frame-by-frame coverage. The desktop recorder stalled opening its capture session and was stopped. All 524 existing release client tests, client checking, and workspace/all-target Clippy passed; no tests were added.

### Settled-memory follow-up (2026-09-18)

Temporary allocation counters, allocation backtraces, asset inventories, Linux process mappings, and wgpu's allocator report separated the memory owners in the same Hotel view. At about 20 seconds, live Rust allocations totaled 519 MiB while mimalloc mappings occupied 1,269 MiB of resident RAM. Native malloc reported another 183 MiB in use. The Rust total included only 14.7 MiB of CPU image buffers and 42.7 MiB of mesh buffers; catalog texture pixels had already been released. Large live allocations included ECS tables and render queues, imported model vertices, mesh-allocation bookkeeping, and physics acceleration structures.

GPU memory was a separate 2.27 GiB allocated out of 2.85 GiB reserved, including textures, mesh slabs, and render targets. It must not be added to process RSS as another CPU allocation. Most of the gap between live Rust allocations and resident mimalloc memory was allocator retention/fragmentation. The host enables transparent huge pages; disabling them for one diagnostic process reduced settled RSS from 1,853 to 1,623 MiB, but did not eliminate the gap. This experiment does not establish a portable performance improvement, and no allocator or operating-system tuning was retained.

One concrete reduction was the wall-light scene structure. Hotel repeats 533 fixtures; the decorative and utility GLBs previously contained 16 and 18 independent nodes/primitives respectively. Their generator now assembles each fixture in world space as one mesh with four material primitives. The diagnostic scene entity count fell from 43,858 to 28,806, with about 50 MiB fewer live Rust allocations. Material definitions, triangle counts, and positions were preserved; corner-normal differences were below 0.0005 after Blender's custom-normal encoding. Matching before/after studio renders confirmed the appearance of both fixtures.

The final comparison removed allocation tracing and retained only a temporary frame-time sampler. Each release run lasted 70 seconds at 1280×720 logical / 1920×1080 render resolution, deferred rendering, mipmaps enabled, and VSync off. The player stood at `--spawn=-60,10,-10 --look=90,-10`, with `--god --peace`. RSS is the mean over seconds 55–70; frame times cover seconds 20–60. No concurrent builds, tests, or other game instances ran during sampling.

| Wall-light models | Peak RSS (MiB) | Settled RSS (MiB) | Mean frame (ms) | Median (ms) | p95 (ms) | p99 (ms) |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Original | 2,382 | 1,913 | 5.04 | 5.04 | 5.91 | 6.41 |
| Consolidated | 2,383 | 1,795 | 4.34 | 4.31 | 5.40 | 5.99 |

This pair saved about 118 MiB of settled RAM (6%) and reduced mean frame time by 14%; loading peak was unchanged. The earlier instrumented pair saved about 55 MiB of RSS, so the exact resident-memory reduction varies with allocator behavior. These are local measurements on one machine and view, not a cross-platform guarantee. All temporary allocator, asset, and frame-time probes were removed.

Final validation: both fixtures were inspected in the running game after removing the probes, including their emissive materials and lighting. All 524 existing release client tests and workspace/all-target Clippy with warnings denied passed. No tests were added. The existing Wayland cursor-position diagnostic remains the only runtime error observed in these checks. The completed profiling follow-up was removed from TODO.
