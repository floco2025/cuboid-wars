...
pub fn actor_spawn_quota_system(
    ...
    let respawn_delay = server_gameplay_config.actors.respawn_delay_seconds;
    ...
    for (zone_idx, zone) in map_config.actor_spawn_zones.iter().enumerate() {
        ...
        let key = (zone_idx, ACTOR_SPAWN_KIND.to_string());
        let throttle = throttles.0.entry(key.clone()).or_insert(0.0);

        match decide_spawn(live, entry.count, *throttle) {
            SpawnDecision::Skip => {
                // Frozen — leave the throttle as-is so the next death pays
                // a full `respawn_delay` wait.
            }
            SpawnDecision::Tick => {
                *throttle -= dt;
            }
