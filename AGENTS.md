...
- **`server/`** — authoritative headless server (Bevy `MinimalPlugins`).
  ...
  - `actors/`, `characters/`, `items/`, `players/`, `projectiles/`, `quests/` — server-side domain systems. Each domain keeps its Bevy resources in its own `resources.rs` (`players/resources.rs` holds `PlayerInfo`/`PlayerMap` and inventory/power-up state, while `players/falling.rs` owns `PlayerFallState`; `quests/resources.rs` holds the session-wide `QuestBoard` and `quests/progress.rs` the `record_player_event` / `record_world_event` entry points). Actor AI uses separate contact/beam controllers over `Roam`/`Engage`/`Evade`/`ReturnHome` modes; navigation precomputes graph-based home/roam territories from spawn zones, and every mode uses the same waypoint route follower.
  - `network/` — the whole networking concern: async QUIC transport (`transport.rs`, accepts connections), Bevy-side message dispatch (`incoming.rs`/`messages.rs`), login, snapshot broadcast (`snapshot.rs`/`broadcast.rs`), the message-feed emitters (`feed.rs` — `announce`/`reply` wrap `SFeed`), and the admin command executor (`admin.rs` — parses `CAdmin` strings like `/give missiles` or `/firework`; `/help` lists all; world-affecting commands announce to everyone, the rest reply to the issuer).
...
Server→client messages have five roles, documented at the top of `common/src/protocol.rs`:
...
4. **Per-client state events** — durable per-player state other clients don't need (e.g. quest assignment/progress), unicast to the affected player with no snapshot fallback. Quest progress is absolute and max-merged; the client buffers progress/completion received before assignment until the assignment supplies display metadata. Group quest state (pooled progress, players done, latched completion) is world state and rides `SSnapshot.quests` instead; `SQuestCompleted` goes to every player for group quests.
5. **Feed lines** (`SFeed`) — server-authored message-feed lines (`FeedEvent`: joins/leaves, deaths with their `DeathCause`, key pickups, kills of actor kinds enabled in `feed.actor_destroyed`, quest completions, barrier openings, admin actions, chat). Names and kinds are resolved server-side at emit time so the client renders without live maps; broadcast except admin replies, which are unicast. The server config's `feed` block is the one switch for which lines are broadcast (`FeedConfig::announces`, consulted by `announce`). Ephemeral like cues — a dropped line costs only the text.
...
`kill_player` in `server/src/combat/damage.rs` is the single entry point — takes a `DeathSource` (who or what, by id), derives the kill credit (`kill_credit`: a shooter other than the victim who is still connected), clears per-life state on `PlayerInfo`, arms `death_timer`, queues the death explosion into `PendingExplosions`, despawns the entity, broadcasts `SPlayerDeath`, and announces the `FeedEvent::PlayerDied` line with the wire `DeathCause`. Called from projectile lethal hits, actor beams, explosion blasts, falls below `CHARACTER_FALL_DEATH_Y` (`players_fall_death_system`), and `/kill`. `explosions_system` drains player and actor blasts to a fixed point; death-blast kills award no kill credit (missile blasts do). `players_respawn_system` ticks the timer and spawns a fresh entity at a spawn zone.
...
`config/server/gameplay.json` `quests` is the catalog; each quest has a `kind` (what advances it), a `scope`, and optionally `requires`. Scopes: `individual` (own progress on `PlayerInfo.quest_states`, own completion), `shared` (one pooled counter on the `QuestBoard`; any player's event advances it; completes once for the group), `everyone` (own progress per player; completes for the group once every logged-in player reached the threshold — the HUD shows `done/players`). `requires` hides a quest until the named `shared`/`everyone` quest completes; then it is assigned to every logged-in player and to later joiners. Group completions are latched for the session, credit every logged-in player with `scoring.quest_completed` once, and reach every client as `SQuestCompleted`. `quests::record_player_event` (cookies, actor kills) and `quests::record_world_event` (the firework launch) are the entry points; `quests::recheck_everyone_quests` re-checks `everyone` quests on disconnect. Kinds advanced by a world event rather than a player (`fireworks`) must be `shared`; a kind may claim the plate purpose that solves it (`QuestKind::plate_purpose`), which keeps those plates locked until the quest unlocks. `/quest` lists the catalog and `/quest <id> [name|@a]` completes one by fiat (`quests::complete_quest`, after `unlock_quest` if it is still locked).
