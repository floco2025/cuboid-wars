...
- **`client/`** — Bevy renderer, input, UI.
  ...
  - `network/` — `routing.rs` unwraps `ServerMessage` envelopes and calls focused domain handlers directly (`players/`, `actors/`, `items/`, `missiles/`); `bootstrap.rs`, `quests.rs`, `snapshot.rs`, and `presentation.rs` own the remaining message handling.
...
Both server and client dispatch decoded wire payloads directly from ingress to one domain handler. Do not re-emit them as Bevy events unless a message gains genuinely independent consumers: ingress is already a Bevy system with world access, so the event layer currently adds no useful fan-out, scheduling, or parallelism and only obscures the receive-to-handler control flow.
