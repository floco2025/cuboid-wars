...
- **Death & respawn**. `kill_player` in `server/src/combat.rs` is the single entry point — clears per-life state on `PlayerInfo`, arms `death_timer`, queues the death explosion into `PendingPlayerExplosions`, despawns the entity, broadcasts `SPlayerDeath`. Called from projectile lethal hits, explosion blasts, player-blast deaths (`players_explosion_system` drains the queue and applies `player.explosion` damage; chains resolve within one drain, blast kills award no kill credit), and falls below `CHARACTER_FALL_DEATH_Y` (`players_fall_death_system`). `players_respawn_system` ticks the timer and spawns a fresh entity at a spawn zone.
