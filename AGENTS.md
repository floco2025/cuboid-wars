...
Q swaps the client-only `WeaponMode`; in portal mode left/right click send `CPortalShot` for ends A/B and the server raycasts the aim (`world_surface_along_ray`) and places the aperture on whatever geometry the ray hits — pure point + outward normal, no wall/floor/ceiling taxonomy. Placement is validated by the shared `compute_portal_placement`: the whole aperture needs solid backing and clear front space, and it must not cover wall lights or — for standable portals — pressure plates. A shot that fails where it lands bumps Portal-2-style to the nearest fitting spot on the plane (`nudged_placement`); only when nothing within reach fits does it fizzle (client: dry-fire + a spark burst at the impact point). The client runs the same check on the same static data before sending, so fire vs dry-fire feedback is immediate and a rejected shot is never sent (`client/src/input/portals.rs`). Each player owns one A/B pair (`server/src/portals/resources.rs`, keyed by owner; re-shooting an end moves it); anyone travels through any complete pair. Portals survive their owner's death, leave on disconnect, and ride `SSnapshot.portals`, with `SPortalOpened` as the placement cue.

The geometry and traversal math is shared in `common/src/physics/portals.rs`: aperture frames (world-up projected onto the plane, the shooter's yaw only where that degenerates), the through-pair transform (a proper rotation, never a mirror), the character trigger (body-face proximity + inward motion; standable portals trigger at rest), and the projectile hop both flight sims rank against their other collision events. Player teleports broadcast `SPlayerTeleport`, a deliberate hard-snap cue — the WHY (sub-threshold reconciliation pushes the correction into the portal's own wall) lives on the message doc. Momentum carries as knockback + vertical velocity; `PlayerLife.portal_cooldown` bounds pair oscillation. On exit the local camera does the Portal-style reorientation: the aim (stored yaw/pitch — the authoritative view source, never read back from the camera transform) jumps to the mapped upright view while `PortalTransitBlend` decays the mapped tilt on top of it (`apply_local_portal_facing` in `client/src/network/players/sync.rs`, blend system in `client/src/players/effects.rs`).
