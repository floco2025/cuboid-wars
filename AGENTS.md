...
- `config/common/gameplay.json` — shared simulation tuning loaded by client and server (player/actor physics, actor `roam_speed`/`active_speed`, player jump speed, the `projectiles` flight block, the `ladders` climb ratio, and the `missiles` block both sides need: lock range, aim-assist radius, ammo cap).
