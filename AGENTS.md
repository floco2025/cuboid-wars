...
Client portal surfaces are off-axis render targets (`client/src/portals/render.rs`). `rendering.portal_recursion_depth` is the number of additional see-through apertures inside the first view (0 still renders the direct view); deepest views run first and fixed camera/surface budgets bound multiplayer cost. The local player's meshes use a separate render layer: portal and shadow cameras include it, the first-person camera excludes it, and top-down includes it.
