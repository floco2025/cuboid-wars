...
- **`common/`** — shared between client and server.
  ...
  - `physics/` — shared player/projectile movement, collision world (incl. per-kind barrier collision groups, plus the non-solid ladder climb volumes in `world/ladders.rs`), barrier passability, spawn validation helpers, missile lock-on acquisition (`lock.rs`, used by the client crosshair and server fire validation), and portal geometry + traversal (`portals/` — aperture frames, placement/overlap validation, character/projectile hops, and the `PortalSet` both sides rebuild from the replicated portal list). Moving floors are the one geometry that moves: `world/moving_floor_sync.rs` places their colliders each tick, and the character step carries whoever stands on one.
  ...
  - `map/` — shared map behaviour: ramp surfaces (`levels.rs`), moving floor poses (`moving_floors.rs` — `surface_center_at`, a pure function of the shared tick, and the `MovingFloors` runtime state both sides advance), and grid↔world conversion (`geometry.rs`, `MapGeometry`, built from the grid size and the map's `MapGeometryConfig`). The per-map cell size, storey height, and floor and wall thicknesses ride `MapSettings.geometry` (`config/geometry.rs`, which derives the wall height, half wall thickness, barrier and bridge thicknesses, `level_y`, `level_for_y`, and `nearest_level_to_y` from them); the barrier and bridge thicknesses are fractions in `constants.rs`, and no other world size is a constant. Layout records are world space — the server fills `y`/`height` on walls, barriers, ladders, and plates at compile time — so the physics and the client place them without the map's sizes and only grid logic needs `MapGeometry`.
...
A tile a little smaller than its cell that slides between the centers of two cells, on one storey or between two (`{level, from, to, to_level, speed, pause_secs, phase_secs}` plus floor materials in the map JSON, top-level like ramps). It compiles to a world-space `MovingFloor` in `MapLayout` with whole-tick timing, and its pose is a pure function of the shared tick (`common/src/map/moving_floors.rs`, `surface_center_at`), so nothing per tile rides the wire. Both sides keep the same `MovingFloors` resource and run the shared `moving_floors_advance_system` right before character movement: it steps every tile to this tick's pose and moves its collider (`CollisionWorld::set_moving_floor_centers`, the one place a collider ever moves). Tiles share `MOVING_FLOOR_COLLISION_GROUP`, a surface for characters and projectiles that is not world geometry, so sight and the portal ray pass through them and no portal can be placed on one. Riding happens inside `step_character_movement`: `MovingFloors::carry_at` says which tile a body stands on and how far it moved (the rule and its WHY live there), the vertical part moves the body before the ground probe, and the horizontal part rides the move so walls still block; a body that leaves a tile keeps its velocity as `AirborneMomentum` (the portal launch velocity, generalized) and its rise or drop in its vertical velocity. Actors are carried like players but keep no momentum when they leave a tile, and never path onto one: like light bridges, tiles set no `Cell` flags. The client spawns one entity per tile (`client/src/moving_floors/`), interpolated between its last two tick poses like a character, and level focus shows it on every storey it spans. Validation keeps a tile's whole path clear of slabs, ramps, bridges, and interior wall or barrier edges (`validate_moving_floors`); a tile sliding into a standing body or descending onto a head is not resolved.
...
- Mesh UVs are computed from world position, not local position. Floor/wall/ramp builders in `client/src/map/spawn/` take `world_center` (and `rotation` for walls); each vertex's UV is `(world_center + rotation * local_pos) · uv_axis / tile_size`. New mesh builders should follow the same pattern; the one exception is a mesh that moves (`client/src/moving_floors/spawn.rs` passes a zero `world_center`), since a world-projected texture would swim across it.
...
mode (floors, grass, walls, ramps, moving floors, ladders, barriers, light
bridges, spawn zones, items, materials, lights, pressure plates). Every element group in
