...
- **Weather & time**. Both concerns are continuous scalars in every snapshot, seeded from the map's `weather`/`lighting` mode (a concrete state, or `auto` for the global cycle). `weather_system` runs the rain cycle; intensity rides `SSnapshot.rain_intensity` and the client smooths + renders it (`vfx/rain.rs`). Lighting is separate — rain does not dim the world. The wire speaks preset names: `SSnapshot.lighting` is a `LightingBlend {from, to, blend}` between two named client-side looks (`bright`/`dim`/`dark`, hand-tuned in `client.json`; a plain preset is the degenerate `from == to`). `light_cycle_system` runs the cycle — a wrapping clock over `lighting_cycle` (hold at each present stop of `bright_secs`/`dim_secs`/`dark_secs` — any two or three — fading between adjacent stops via `bright_dim_secs`/`dim_dark_secs`, or `bright_dark_secs` when dim is skipped, down and back up; `blend_at` is the pure timeline→blend map) — and the client's `lighting_blend_system` resolves the names and eases every channel toward the blended look in look space, so cycle steps, segment crossings, and admin jumps all fade with one mechanism. `/light` reports the current blend; `/light bright|dim|dark` holds a preset absolutely, `/light <0..1>` holds a fraction of the cycle's descending range, `/light <from> <to> <0..1>` holds an arbitrary blend, `/light auto` resumes the cycle (continuously when the held blend lies on its path). `/weather` likewise reports current weather. `/weather rain|clear|auto` and `/light bright|dim|dark|auto|<0..1>` hold a state or resume the cycle continuously. The login-time snapshot carries the live values, so a dark map never flashes bright on connect.
...
- **Actor AI**. Contact attackers, beam attackers, and contact+beam attackers have separate decision controllers selected by the tagged `combat.attack` config (`contact`, `beam`, `contact_beam`). Decisions run at 10 Hz while route following, collision, beam damage, and timers remain at the 30 Hz server tick. Spawn zones expand through the floor-walking `NavGraph` by `roam_steps` into a precomputed home/roam territory; active combat is not leashed to it and may use the actor's entire reachable nav component. Engagement routes string-pull BFS cell waypoints across footprint-safe flat floor, so actors steer directly in open space while retaining turns around obstacles and level-transition waypoints. Two rules keep route following from jamming, because the motor parks on a blocked step rather than scraping along walls: every installed route is anchored (`NavGraph::anchor_route_start` — a first leg that would sweep the body through a wall end detours via the current cell centre), and a waypoint the actor has already overshot along the next leg is skipped rather than walked back to (`waypoint_passed` in `behavior/tick.rs` — the ramp-top transition waypoint sits at the actor's own cell centre, so two actors in a one-cell trench otherwise deadlock). LOS within spherical vision acquires and refreshes threats; after LOS or range is lost, actors retain only the last visible state until global `actor_settings.threat_memory_secs` elapses, then return home. Reachable attacks outrank evasion, while contact actors evade players they cannot reach and zappers evade during beam cooldown. Contact+beam actors (the reaper) move by the contact rules in every beam state and fire the beam opportunistically on top; mid-burst they never run for cover, holding an unreachable target in view instead. The beam's target lives in `BeamState::Firing`, independent of the movement mode. Evasion searches for stable non-ramp cover within a bounded local graph radius, revalidates the destination as threats move, and holds it until an attack becomes possible or the threats are forgotten.
