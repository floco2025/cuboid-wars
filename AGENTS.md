...
- `config/client/client_local.json` — the settings menu's saved values, written on menu close (gitignored; versioned — a stale version is discarded, since `git pull` cannot update an ignored file).
