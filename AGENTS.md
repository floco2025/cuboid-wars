...
The authoritative server→client message taxonomy is the top-of-file comment in `common/src/protocol.rs`; keep the roles and their detailed rules there only. Read it before adding a message. Most “X changed” state belongs in `SSnapshot`; add a one-shot only for sub-tick latency, an edge-triggered side effect, or information a snapshot cannot carry.
