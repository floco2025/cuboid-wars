...
- **Weather & time**. `weather_system` runs each map's optional `rain` schedule (or `/weather rain|clear` overrides); intensity rides `SSnapshot.rain_intensity` and the client smooths + renders it (`vfx/rain.rs`). Lighting is separate: `/time day|night` rides `SSnapshot.time_of_day` (`night_dim_system`) — rain does not dim the world.
