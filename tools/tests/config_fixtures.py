...
def gameplay():
    return {
        "default_map": "hotel", "maps": ["hotel", "obby"],
        "actors": {"kinds": {name: {"immovable": name == "turret"} for name in
                             ("scuttler", "bruiser", "zapper", "turret")}},
...
def map_settings(name="hotel"):
    return {
        "geometry": {"grid_cell_size": 3.4, "level_height": 4.4,
                     "floor_thickness": 0.4, "wall_thickness": 0.3},
        "movement": {"gravity": 25, "low_gravity": 5,
                     "player": {"walk_speed": 6, "run_speed": 9,
                                "speed_power_up": 1.5, "jump_speed": 12}},
        "player_fall": {"safe_distance": 8, "lethal_distance": 15},
        "textures": {alias: {"portalable": alias != "portal-resistant"} for alias in
                     (["basement-floor", "floor-a", "floor-b", "slab", "upper-floors", "wall"] if name == "hotel" else
                      ["basement-floor", "portal-resistant", "slab"])},
        "barrier_kinds": ([{"id": kind, "color": color} for kind, color in
                           [("treasure", "#ff3333"), ("basement", "#f0c020"),
                            ("gravity", "#5090ff"), ("lobby", "#22cc33")]] if name == "hotel" else
...
def install_catalogs(test, root):
    ...
    test.assets_path.write_text(json.dumps({
