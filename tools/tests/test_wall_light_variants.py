...
from map_editor.constants import MODE_LIGHT
...
class WallLightVariantTests(WindowTestCase):
    def test_selected_variant_survives_save_reload_and_undo(self):
        ...
        self.assertIn(
            "utility", element_hover_text(window.map_data, 0, ("Light", (1, 1, "N")))
        )
    ...
    def test_unknown_style_is_reported_and_preserved_for_repair(self):
        window = self.window
        window.add_wall_line((1, 1), (2, 1))
        data = read_map(window.path)
        data["levels"][0]["walls"] = window.map_data["levels"][0]["walls"]
        data["levels"][0]["lights"] = [
            {"col": 1, "row": 1, "side": "N", "kind": "unknown"}
        ]
        errors = window.validate(data)
        self.assertTrue(any("unknown kind 'unknown'" in error for error in errors))
        write_map(window.path, data)
        self.assertEqual(
            read_map(window.path)["levels"][0]["lights"][0]["kind"], "unknown"
        )
