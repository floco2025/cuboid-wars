...
class Selection:
    ...
    def refreshed(self, before, after):
        ...
        records = [(ref.name, ref.level, ref.get(before)) for ref in self.objects]
        refs = tuple(ref for ref, entry in element_refs(after) if (ref.name, ref.level, entry) in records)
        anchor = self.anchor
