...
class EraseMixin:
    ...
    def erase_cell_rect(self, start: tuple[int, int], end: tuple[int, int], preserve_floors: bool) -> None:
        ...
        self.apply_change(
            label, erasing.erase_cell_rect(self.editable_map_data(), self.current_level, start, end, preserve_floors)
        )
    ...
    def erase_group_rect(self, mode: str, start: tuple[int, int], end: tuple[int, int]) -> None:
        ...
        after = erasing.erase_group_rect(
            self.editable_map_data(), mode, self.current_level, rect_from_cells(start, end)
        )
        if after is None:
    ...
    def hit_at(self, pos):
        return erasing.hit_at(
            self.visible_map_data(), self.current_level, pos.x(), pos.y(), self.canvas.pick_tolerance()
        )

    def erase_hit(self, hit, preserve_floors: bool = False) -> None:
        if any(
            ref.name in self.element_filters.excluded for ref in refs_for_hit(self.map_data, self.current_level, hit)
        ):
            self.notify("The selected element type is hidden or locked.")
            return
        if hit[0] in (HIT_SPAWN_ZONE, HIT_CHECKPOINT) and self.selected_spawn_zone_ref == ZoneRef(*hit[1]):
            self.selected_spawn_zone_ref = None
        self.apply_change(
            f"Erase {hit[0]}", erasing.erase_hit(self.editable_map_data(), self.current_level, hit, preserve_floors)
        )
