...
class MotionDialog(QDialog):
    ...
    of its far end, its speed, the pause at each end, and the phase offset
    of its cycle. `recent` is the previous answer, remembered for the next
    placement."""
    ...
    def __init__(
        ...
        level_count: int,
        current_level: int,
        recent: tuple[str, int, float, float, float] | None,
        title: str,
        map_names: list[str],
    ):
        super().__init__(parent)
        ...
        to_level, speed, pause_secs, phase_secs = recent[1:] if recent else (current_level, 2.0, 1.0, 0.0)
        self._map = QComboBox()
    ...
    def motion(self) -> tuple[int, float, float, float]:
