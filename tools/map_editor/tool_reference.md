...
- **Barrier** — Choose Kind and **Controls…** in the toolbar, then drag along grid lines. Controls choose **Initial state** and an optional switch: On is solid, Off is passable, and the switch flips that state while it is active, so a door a plate opens starts On; without a switch the barrier keeps its initial state. Every barrier also accepts its matching key automatically. Select one or more barriers to edit their shared Properties.
- **Erase Barriers** — Drag a rectangle to remove every barrier edge inside or on its border.

## Light Bridges

- **Light Bridge** — Choose Kind and **Controls…** in the toolbar, then drag cells. Controls choose **Initial state** and an optional switch: On is a solid walkway, Off a passable ghost, and the switch flips that state while it is active; without a switch the bridge keeps its initial state. A key of the bridge's kind drops its holder through it. Select one or more bridges to edit their shared Properties. Bridges cannot share cells with floors or ramps.
...
Each tile can hold one pressure plate per level, on a floor or blocked floor outside ramp footprints. Several plates may operate the same switch and its targets. **Map → Switches** creates, edits, renames, and deletes switches. The root layout stores their policies and all target assignments; nested maps share the root catalog. Rename updates all references, and a switch still in use cannot be deleted. The **Used by** column counts the plates and targets naming each switch across the outer map and every nested map; **Unused** marks one nothing names.

Each switch chooses `momentary`, `toggle`, or `auto` activation, a death reset rule (`never`, `solo`, `any`, or `all`), and `any` or `everyone` holding. Auto toggles with one logged-in player and is momentary with several. Everyone requires each living player on a plate, or every plate occupied when players outnumber them. The optional color override is picked from a swatch; otherwise plates take the first field kind, in catalog order, that a barrier or bridge on the switch uses, then the default fixture color.

Barriers, bridges, actor zones, and nested maps choose an **Initial state** and optionally one switch; **Map → Fireworks** chooses the switch alone and a cooldown. The initial state is the target's state before any switch input, and an assigned switch flips it while active: On is a solid barrier or bridge, a spawning zone, a running Cycle map, or a Follow switch map at end 2. New placements start On, and a target without a switch, or on a switch no plate operates yet, keeps its initial state. Fireworks need an assignment. **Map → Field Kinds** edits the layout's one appearance catalog, which barriers, light bridges, and keys share; each kind's color is picked from a swatch, a key opens every barrier of its kind and drops its holder through every light bridge of that kind, and **Used by** counts the barriers, bridge cells, and keys of each kind. Catalog changes share Save, Undo, and autosave recovery with the layout.
