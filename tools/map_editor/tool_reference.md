...
- **Checkpoint** — Set the Number and choose Type in the toolbar, then drag a rectangle. The toolbar offers the map's next free number and advances after each placement; a number already used anywhere in the map is refused. Individual saves the entrant's own respawn point, Group — any saves everyone's when one player enters, and Group — all waits until every player has visited. Select it to move it, resize with its handles, or change its number or type in Properties; Delete removes it. The number shows on the canvas, orders the course (a player only ever advances to a higher number), ends the actor zones that name it, and lets the game start or respawn there (`--checkpoint <number>` at launch, `/checkpoint <number>` in the console). A copied checkpoint gets a fresh number, and the actor zones copied with it follow. **Map → Edit Checkpoints…** lists every checkpoint of the map in order to renumber or reorder them; actor zones ending at a renumbered checkpoint follow it.
