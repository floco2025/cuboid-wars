...
- **Barrier** — Choose Kind and **Controls…** in the toolbar, then drag along grid lines. Controls choose a switch and **Solid when**: the barrier is solid while the switch is in that state and open otherwise, so a door a plate opens is solid when Off; without an assignment it stays solid. Every barrier also accepts its matching key automatically. Select one or more barriers to edit their shared Properties.
- **Erase Barriers** — Drag a rectangle to remove every barrier edge inside or on its border.

## Light Bridges

- **Light Bridge** — Choose Kind and **Controls…** in the toolbar, then drag cells. Controls choose a switch and **Solid when**: the walkway is powered while the switch is in that state; without an assignment it is unpowered. Select one or more bridges to edit their shared Properties. Bridges cannot share cells with floors or ramps.
...
Barriers, bridges, actor zones, and moving nested maps choose one switch and an On/Off response; **Map → Fireworks** chooses the switch alone and a cooldown. For barriers and bridges the response is **Solid when**: the field is solid while the switch is in that state and passable or unpowered otherwise, so a door opened by a plate is solid when Off, a bridge powered by one is solid when On, and a barrier and a bridge with the same setting are solid together. New assignments start at On. Without an assignment, barriers stay closed, bridges unpowered, actor zones active, and Cycle maps running. Follow switch requires an assignment. Fireworks need an assignment. **Map → Barrier Kinds** and **Bridge Kinds** edit the layout's appearance catalogs; each kind's color is picked from a swatch, keys always match barrier kinds, and **Used by** counts the barriers and keys, or bridge cells, of each kind. Catalog changes share Save, Undo, and autosave recovery with the layout.
