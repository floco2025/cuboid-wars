...
- **Map** — Open a map registered by name in `gameplay.json`. Its folder contains `layout.json` for placement and the switch and field catalogs, and `settings.json` for tuning. The Map picker switches between the outer map and its named nested geometry; all views use the parent's fields, switches, and texture catalog. Save, autosave, and undo cover the whole document. Undo switches to the affected map.
...
- **Hover details** — Hover any element to see its type and relevant properties, including materials, a barrier's or bridge's field with its switch and initial state, actor count, or nested-map motion. Overlapping elements follow the same priority as right-click; material tools show their target's materials.
- **Check Map / Review Repairs** — Map → **Check Map…** opens a separate issues window when needed. Click a result to select its map and level and highlight the object. Warnings come last: they name something that does nothing until another record exists, such as a switch no pressure plate operates yet, a zone ending at a checkpoint not yet placed, or a Follow switch map with no switch, and they never block an edit, a save, or the game loading the map, so nothing has to be built in a particular order. Loading preserves invalid records and warns about errors; **Review Repairs** lists automatic changes for approval. Accepted repairs undo in one step. While repairs are pending, edits preserve records for manual correction; saving remains blocked by validation errors.
...
- **Sample** — Choose the eyedropper and click an element, press `I` while pointing at it with canvas focus, or right-click → **Use This Tool**. This selects its placement tool and reuses its applicable face materials, field, actor kind, count, span, and pressure plate controls. Choosing a new material clears the sampled face recipe.
...
- **Actor Spawn Zone** — Choose Actor and Count in the toolbar, then drag a rectangle. **Controls…** sets First level, Levels, Roam extension in metres (default 0, inside the zone), Respawn, the delay before a killed actor's slot refills (Never fills the zone once), Initial state and an optional switch, and Active until checkpoint (Always keeps the zone open) with Then, whether the zone only stops spawning or also self-destructs its actors once any player reaches that checkpoint; new zones reuse those values, and right-click edits existing zones. A zone spawns while it is on: **Initial state** is its state before any switch input, and an assigned switch flips it while active, so a zone that starts Off with no switch never spawns. A zone held back keeps counting down and refills once it is on again. Count applies once across all levels. Immovable actors spawn at cell centers; surface suitability is left to the map designer. View → Show Roam Extensions (`R`, initially disabled) toggles the rounded roaming boundary.
...
- **Barrier** — Choose its Field in the toolbar, then drag along grid lines. A barrier is a piece of its field and has no state of its own: it is solid while the field is On and passable while it is Off, and **Map → Fields** sets the field's initial state and optional switch. A key of the field lets its holder through automatically. Select one or more barriers to change their Field in Properties.
- **Erase Barriers** — Drag a rectangle to remove every barrier edge inside or on its border.

## Light Bridges

- **Light Bridge** — Choose its Field in the toolbar, then drag cells. A bridge cell is a piece of its field and has no state of its own: it is a solid walkway while the field is On and a passable ghost while it is Off, and **Map → Fields** sets the field's initial state and optional switch. A key of the field drops its holder through it. Select one or more bridges to change their Field in Properties. Bridges cannot share cells with floors or ramps.
...
- **Nested Map** — Click to place named nested geometry with its cell (0, 0) on that cell; drag to set a different second endpoint. The toolbar's **Settings…** configures new placements; Properties edits selected placements. Both show Map, Motion, Travel time, Cycle pause, Cycle phase, Switch, Initial state, End 2 level, and endpoint nudges in that order.
  - **Cycle** repeats between endpoints, pausing at each end for **Cycle pause**. **Cycle phase** offsets its starting point in that cycle. It runs while it is on: **Initial state** Off starts it paused, and an optional switch flips that state while active, pausing and resuming it where it is.
  - **Follow switch** heads for end 2 while it is on and for end 1 otherwise: **Initial state** On starts it at end 2, Off at end 1, and the switch flips that state while active. It holds on arrival and reverses immediately from its current position when the switch changes; without a switch it rests at its initial end. Cycle pause and phase stay visible but greyed out; their values are preserved when switching modes.
  - **Travel time** is the time for the full distance between endpoints in either mode.
...
Each tile can hold one pressure plate per level, on a floor or blocked floor outside ramp footprints. Several plates may operate the same switch and its targets. **Map → Switches** creates, edits, renames, and deletes switches. The root layout stores their policies and all target assignments; nested maps share the root catalog. Rename updates all references, and a switch still in use cannot be deleted. The **Used by** column counts the plates, fields, and other targets naming each switch across the outer map and every nested map; **Unused** marks one nothing names.

Each switch chooses `momentary`, `toggle`, or `auto` activation, a death reset rule (`never`, `solo`, `any`, or `all`), and `any` or `everyone` holding. Auto toggles with one logged-in player and is momentary with several. Everyone requires each living player on a plate, or every plate occupied when players outnumber them. The optional color override is picked from a swatch; otherwise plates take the color of the first field, in catalog order, that names the switch, then the default fixture color.

Fields, actor zones, and nested maps choose an **Initial state** and optionally one switch; **Map → Fireworks** chooses the switch alone and a cooldown. The initial state is the target's state before any switch input, and an assigned switch flips it while active: On is a solid field, a spawning zone, a running Cycle map, or a Follow switch map at end 2. New targets start On, and a target without a switch, or on a switch no plate operates yet, keeps its initial state. Fireworks need an assignment.

A field is a named thing with one state, like a switch: **Map → Fields** creates, edits, renames, and deletes the root layout's fields, each with a Name, a Color picked from a swatch, an optional Switch, and an Initial state. Barriers, light bridge cells, and keys only name their field, so every barrier and bridge of a field is solid or passable together, and a key lets its holder through all of them. Rename updates every barrier, bridge, and key across the outer map and every nested map, a field still in use cannot be deleted, and **Used by** counts the barriers, bridge cells, and keys naming each field. Catalog changes share Save, Undo, and autosave recovery with the layout.
