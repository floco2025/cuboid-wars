"""Pure erasure and object picking in map coordinates."""
...
def hit_at(data: dict, level_idx: int, pos, cell_size: float):
...
def erase_hit(data: dict, level_idx: int, hit, preserve_floors: bool = False) -> dict:
    ...
    if kind == MODE_GRASS:
        ...
        level["floors"] = [floor for floor in level["floors"] if (floor["col"], floor["row"]) != value]
    elif kind == MODE_INACCESSIBLE_FLOOR:
        level["inaccessible_floors"] = [
            floor for floor in level["inaccessible_floors"] if (floor["col"], floor["row"]) != value
        ]
