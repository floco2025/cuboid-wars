...
use super::{CarrierId, PortalPairId, Position};
...
// One placed portal end: surface geometry in its carrier's frame. `pos` is
// the aperture center and `(nx, ny, nz)` the outward unit normal, both local
// to `carrier` (world space on `CarrierId::WORLD`), so the wire value stays
// the same while the carrier moves and both sides place the aperture from
// their own carrier poses. `yaw` is the shooter's facing at placement; it
// orients the aperture frame only where world-up is degenerate
