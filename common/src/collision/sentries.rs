...
use crate::{
    constants::{SENTRY_SIZE, PLAYER_DEPTH, PLAYER_HEIGHT, PLAYER_WIDTH},
    protocol::{Position, Ramp, Wall},
};

#[must_use]
pub fn overlap_sentry_vs_wall(sentry_pos: &Position, wall: &Wall) -> bool {
    let sentry_half_size = SENTRY_SIZE / 2.0;
    overlap_aabb_vs_wall(sentry_pos, wall, sentry_half_size, sentry_half_size)
}

#[must_use]
pub fn sweep_sentry_vs_wall(start_pos: &Position, end_pos: &Position, wall: &Wall) -> bool {
    let sentry_half_size = SENTRY_SIZE / 2.0;
    sweep_aabb_vs_wall(start_pos, end_pos, wall, sentry_half_size, sentry_half_size)
}
...
pub fn overlap_sentry_vs_ramp_footprint(sentry_pos: &Position, ramp: &Ramp) -> bool {
    ...
    let g_min_x = sentry_pos.x - half;
    let g_max_x = sentry_pos.x + half;
    let g_min_z = sentry_pos.z - half;
    let g_max_z = sentry_pos.z + half;
...
pub fn overlap_sentry_vs_player(sentry_pos: &Position, player_pos: &Position) -> bool {
    ...
    if y_diff > f32::midpoint(PLAYER_HEIGHT, SENTRY_SIZE) {
        return false;
    }

    let player_half_x = PLAYER_WIDTH / 2.0;
    let player_half_z = PLAYER_DEPTH / 2.0;
    let sentry_half = SENTRY_SIZE / 2.0;
    ...
    let g_min_x = sentry_pos.x - sentry_half;
    let g_max_x = sentry_pos.x + sentry_half;
    let g_min_z = sentry_pos.z - sentry_half;
    let g_max_z = sentry_pos.z + sentry_half;
