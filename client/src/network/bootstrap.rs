...
pub(super) fn handle_bootstrap_message(
    message: &SInit,
    ...
    commands.insert_resource(message.map_layout.clone());
    commands.insert_resource(message.map_settings.clone());
    commands.insert_resource(collision_world);
    commands.insert_resource(BlastRadii {
        actors: message.actor_blast_radii.iter().cloned().collect(),
    ...
    commands.insert_resource(MaxHealth {
        ...
        actors: message.actor_max_health.iter().cloned().collect(),
    });
}
