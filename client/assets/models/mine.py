"""Build the mine GLB in Blender; add -- --preview [--motion] for previews in /tmp."""
...
armature = bpy.data.armatures.new("Mine mechanism")
rig = bpy.data.objects.new("MineRig", armature)
...
print("Exported mine:", MODEL.stat().st_size, "bytes")
print(
    "Bounds:",
    tuple(
        round(
...
if "--preview" in sys.argv:
    ...
    scene.render.filepath = "/tmp/mine-preview.png"
    bpy.ops.render.render(write_still=True)
    if "--motion" in sys.argv:
        output = Path("/tmp/mine-motion")
