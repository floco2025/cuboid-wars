"""Build the player GLB in Blender; add -- --preview [--rear] for stills in /tmp."""
...
from player_mocap import PlayerMocap
...
def box(name, pos, size, mat, bone, bevel=0.018):
    return primitives.box(name, pos, size, mat, rigged(parts, bone), bevel, 3)


def sphere(name, pos, size, mat, bone):
    return primitives.sphere(name, pos, size, mat, rigged(parts, bone), 16, 8)
...
def shell(name, x, rings, mat, bone, arc=(0, math.tau), exponent=1.0):
    ...
    if not closed:
        ...
        bpy.ops.object.modifier_apply(modifier=modifier.name)
    return primitives.finish(obj, name, mat, rigged(parts, bone), 0, 0, "all")
...
def cable(name, points, radius, mat, bone):
    ...
    return primitives.finish(bpy.context.object, name, mat, rigged(parts, bone), 0, 0)
...
for sign in (-1, 1):
    shell(
        ...
        sign * 0.125,
        [
            (0.88, 0.019, 0.07, 0.02),
            (0.91, 0.035, 0.085, 0.02),
            (0.958, 0.035, 0.074, 0.02),
            (0.97, 0.024, 0.06, 0.02),
        ],
        ivory,
        "Root",
    )
...
for sign in (-1, 1):
    rod(
        ...
        (sign * 0.093, 0.008, 0.97),
        (sign * 0.127, 0.018, 1.13),
        0.012,
        steel,
        "Torso",
    )
    cable(
        ...
        [
            (sign * 0.055, 0.066, 0.97),
            (sign * 0.067, 0.081, 1.025),
            (sign * 0.080, 0.072, 1.09),
        ],
        0.009,
        joint,
        "Torso",
...
for sign in (-1, 1):
    for z in (1.16, 1.19, 1.22):
        cable(
            ...
            [
                (sign * 0.144, -0.058, z),
                (sign * 0.161, 0, z + 0.014),
                (sign * 0.153, 0.065, z + 0.016),
            ],
            0.008,
            joint,
            "Torso",
    ...
    cable(
        ...
        [
            (sign * 0.145, 0.06, 1.427),
            (sign * 0.218, 0.073, 1.43),
            (sign * 0.255, 0.053, 1.38),
        ],
        0.009,
        joint,
        "Torso",
...
shell(
    ...
    0,
    [
        (1.531, 0.046, 0.040, 0.055),
        (1.558, 0.059, 0.05, 0.053),
        (1.61, 0.067, 0.055, 0.04),
    ],
    chassis,
    "Head",
    (0.10, math.pi - 0.10),
...
for side, sign in (("L", -1), ("R", 1)):
    ...
    cable(
        "Thigh panel seam",
        [
            (x + sign * 0.057, 0.025, 0.64),
            (x + sign * 0.064, 0.03, 0.74),
            (x + sign * 0.055, 0.03, 0.82),
        ],
        0.0025,
        chassis,
        hip,
    ...
    cable(
        ...
        [
            (x - sign * 0.021, -0.028, 0.195),
            (x - sign * 0.031, -0.039, 0.31),
            (x - sign * 0.03, -0.045, 0.41),
        ],
        0.002,
        chassis,
        knee,
    ...
    rod(
        "Ankle linear actuator",
        (x + sign * 0.033, 0.017, 0.172),
        (x + sign * 0.048, 0.038, 0.378),
        0.008,
        steel,
        knee,
    )
    ...
    cable(
        "Triceps cable",
        [
            (sx + sign * 0.025, 0.053, 1.354),
            (ex + sign * 0.021, 0.05, 1.27),
            (ex + sign * 0.023, 0.034, 1.14),
        ],
        0.006,
        joint,
        upper,
    ...
    rod(
        "Radial strut",
        (ex + sign * 0.021, 0, 1.061),
        (wx + sign * 0.021, 0, 0.858),
        0.012,
        steel,
        forearm,
    )
    ...
    for index, offset in enumerate((-0.029, -0.010, 0.010, 0.029)):
        ...
        rod(
            "Distal phalanx",
            (fx, -0.007, 0.727 - length * 0.53),
            (fx, -0.010, 0.73 - length),
            0.0075,
            ivory,
            tip,
        )
    ...
    rod(
        "Thumb metacarpal",
        (wx + sign * 0.032, -0.003, 0.785),
        (wx + sign * 0.056, -0.01, 0.758),
        0.011,
        ivory,
        hand,
    )
    ...
    rod(
        "Thumb phalanx",
        (wx + sign * 0.056, -0.011, 0.758),
        (wx + sign * 0.056, -0.028, 0.732),
        0.009,
        ivory,
        hand,
    )
...
parts.append(bake_wear(armour_parts, ivory, palette.wear, MODEL))
...
for side, sign in (("L", -1), ("R", 1)):
    ...
    for index, offset in enumerate((-0.029, -0.010, 0.010, 0.029)):
        ...
        bone_specs.append(
            (
                f"FingerTip.{index}." + side,
                (sign * 0.303 + offset, -0.006, 0.730 - length * 0.53),
                "Fingers." + side,
            )
        )
...
if "--preview" in sys.argv:
    ...
    preview.floor(plain_material("Studio floor", (0.075, 0.10, 0.115), 0.1, 0.65))
    camera = preview.studio(scene, (0, 0, 0.91), max(high - low))
    preview.aim(camera, (2.6, -4.4, 2.5), (0, 0, 0.91))
    ...
    scene.frame_set(14)
    preview.aim(camera, (1.25, -2.6, 1.63), (0, 0, 1.44))
    camera.data.ortho_scale = 0.95
    scene.render.resolution_x = scene.render.resolution_y = 1100
    preview.render(scene, "/tmp/player-detail.png")

    if "--rear" in sys.argv:
        preview.aim(camera, (0.75, 2.6, 1.85), (0, 0, 1.65))
        camera.data.ortho_scale = 0.52
        preview.render(scene, "/tmp/player-rear-head.png")
