...
`materials.accent.color` in [pressure_plate.json](pressure_plate.json) sets the tread color; `materials.light` controls the active strips and indicator. All surfaces use plain materials. `--preview` reimports the GLB and renders blue, amber, and violet plates in both states to `/tmp/pressure-plate-preview.png`, plus a closer pair in `/tmp/pressure-plate-detail.png`. The client loads the model through `config/client/assets.json::pressure_plate`; the editor draws its symmetric outline with the same tread color. Runtime tread colors inherit a linked barrier or bridge kind, with `settings.json::switches[].plate_color` overriding that color and `pressure_plate.default_color` supplying the fallback. Runtime light color and brightness use `pressure_plate.light_color` and `pressure_plate.emissive_luminance`.
